...
#define AttemptSemaphoreShared AttemptSemaphoreSharedPPC
...
#define SetTaskPri SetTaskPriPPC
#define Wait WaitPPC
#define AllocSignal AllocSignalPPC
#define SetSignal SetSignalPPC
#define FreeSignal FreeSignalPPC
#define SetExcept SetExceptPPC
#define NEWLIST NewListPPC
#define DELAYTIME 1000000 // 1s delay before repolling number of running tasks
#define STACK_DEFSIZE 0x10000
#endif
...
static int SemaphoreIsInvalid(struct SignalSemaphore *sem)
{
	...
	#ifdef WARPUP
	return (!sem || sem->ssppc_SS.ss_Link.ln_Type != NT_SIGNALSEM || sem->ssppc_SS.ss_WaitQueue.mlh_Tail != NULL);
	#else
...
static int SemaphoreIsMine(struct SignalSemaphore *sem)
{
	...
	#ifdef WARPUP
	return (sem && sem->ssppc_SS.ss_NestCount > 0 && (struct Task*)sem->ssppc_SS.ss_Owner == me);
	#else
...
#ifdef WARPUP
	struct SignalSemaphore *sem = &mutex->semaphore;
	sem->ssppc_SS.ss_Link.ln_Type = NT_SIGNALSEM;
#endif
	mutex->incond = 0;
...
int pthread_mutex_destroy(pthread_mutex_t *mutex)
{
	...
	ReleaseSemaphore(&mutex->semaphore);
#ifdef WARPUP
	FreeSemaphorePPC(&mutex->semaphore);
#endif
...
#ifdef WARPUP
	struct SignalSemaphore *sem = &cond->semaphore;
	sem->ssppc_SS.ss_Link.ln_Type = NT_SIGNALSEM;
#endif
...
int pthread_cond_destroy(pthread_cond_t *cond)
{
	...
	ReleaseSemaphore(&cond->semaphore);
#ifdef WARPUP
	FreeSemaphorePPC(&cond->semaphore);
#endif
...
static int _pthread_cond_timedwait(pthread_cond_t *cond, pthread_mutex_t *mutex, const struct timespec *abstime, BOOL relative)
{
	...
	if (abstime)
	{
		...
		if (!relative)
		{
			...
			// GetSysTime can't be used due to the timezone offset in abstime
			gettimeofday(&starttime, NULL);
			timersub(&wantedtime, &starttime, &wantedtime);
		}
	}

	ULONG waittime = (wantedtime.tv_sec * 1000000) + wantedtime.tv_usec;
...
#ifdef WARPUP
	struct SignalSemaphore *sem = &lock->semaphore;
	sem->ssppc_SS.ss_Link.ln_Type = NT_SIGNALSEM;
#endif
	return 0;
}
...
#ifdef WARPUP
	FreeSemaphorePPC(&lock->semaphore);
#endif
...
int pthread_rwlock_unlock(pthread_rwlock_t *lock)
{
	...
	#ifdef WARPUP
	if (lock->semaphore.ssppc_SS.ss_NestCount < 1)
	#else
...
int pthread_attr_init(pthread_attr_t *attr)
{
	...
	task = FindTask(NULL);
	#ifdef WARPUP
	attr->param.sched_priority = task->tp_Task.tc_Node.ln_Pri;
	attr->stacksize = task->tp_StackSize;
	#else
...
#ifdef WARPUP
ULONG CancelHandlerFunc(void)
{	
	register APTR loaddata asm("r2");
	register ULONG loadsignal asm("r3");

	ULONG signals = loadsignal;
	APTR data = loaddata;

	DB2(bug("%s(%u, %p, %p)\n", __FUNCTION__, signals, data));
...
static void StarterFunc(void)
{
	ThreadInfo *inf;
#else
static void StarterFunc(ThreadInfo *inf)
{
#endif
	int i, j;
	int foundkey = TRUE;
	...
	#else
	inf->task = FindTask(NULL);				//Doing it the WarpOS way...
	#endif
	// trim the name
...
#elif WARPUP
	oldexcept = inf->task->tp_Task.tc_ExceptCode;
	inf->task->tp_Task.tc_ExceptCode = &CancelHandler;
#else
	oldexcept = inf->task->tc_ExceptCode;
...
#ifdef WARPUP
	inf->task->tp_Task.tc_ExceptCode = oldexcept;
#else
...
#ifdef WARPUP
	struct TagItem tasktags[6] = {
		{TASKATTR_CODE,		(ULONG) StarterFunc},
		{TASKATTR_NAME,		(ULONG) name},
		{TASKATTR_PRI,		inf->attr.param.sched_priority},
		{(inf->attr.stackaddr == NULL && inf->attr.stacksize > 0) ? TASKATTR_STACKSIZE : STACK_DEFSIZE, inf->attr.stacksize},
		//{TASKATTR_INHERITR2, TRUE},
		{TASKATTR_R3, (ULONG) inf},
	 	{TAG_END,0}
    		};
	inf->task = (struct Task *)CreateTaskPPC(tasktags);
#else	
...
#ifdef WARPUP
	while ((handler = (CleanupHandler *)RemHeadPPC((struct List *)&inf->cleanup))) //WarpOS RemTail broken
#else
...
//	{
//	while (runningtasks)
//	  WaitTime(0, DELAYTIME);		//DEBUGDEBUG Need time-out?
//	}
#endif
}
...
void pthread_cleanup_push(void (*routine)(void *), void *arg)
{
	...
	handler->arg = arg;
#ifdef WARPUP
	AddHeadPPC((struct List *)&inf->cleanup, (struct Node *)handler);   //WOS has bug in RemTailPPC
#else
...
void pthread_cleanup_pop(int execute)
{
	...
	inf = GetThreadInfo(thread);
#ifdef WARPUP
	handler = (CleanupHandler *)RemHeadPPC((struct List *)&inf->cleanup);	//RemTail WOS bug.
#else
...
#ifdef WARPUP
	struct SignalSemaphore *sem1 = &thread_sem;
	sem1->ssppc_SS.ss_Link.ln_Type = NT_SIGNALSEM;
	struct SignalSemaphore *sem2 = &tls_sem;
	sem2->ssppc_SS.ss_Link.ln_Type = NT_SIGNALSEM;
#endif
...
void _Exit_Func(void)
{
	...
	FreeSemaphorePPC(&thread_sem);
	FreeSemaphorePPC(&tls_sem);
#endif
}
